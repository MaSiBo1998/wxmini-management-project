...
function CRUD(options) {
  ...
  const data = {
    ...
    status: {
      ...
      // 标题
      get title() {
        return this.add > CRUD.STATUS.NORMAL ? `${i18n.t('btn.add')}${crud.title}` : this.edit > CRUD.STATUS.NORMAL ? `${i18n.t('btn.edit')}${crud.title}` : crud.title
      }
    },
    msg: {
      submit: '提交成功',
  ...
  const methods = {
     ...
     * 通用导出
     */
    doExport(url, exportType, dayOrMonth) {
      crud.downloadLoading = true
      download(url, crud.getQueryParams(), exportType, dayOrMonth).then(result => {
        console.log(result)
        const name = decodeURIComponent(result.headers['content-disposition'].split('"')[1])
        downloadFile(result.data, name, 'xlsx')
